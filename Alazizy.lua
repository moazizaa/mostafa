...
local N = (Redis:get(Alazizy.."Name:Bot") or "العزايزي")
...
if text == 'رفع نسخه العزايزي' and msg.reply_to_message_id ~= 0 then
if not msg.ControllerBot then 
return LuaTele.sendText(msg_chat_id,msg_id,'\n*⋆ هاذا الامر يخص { '..Controller_Num(1)..' }* ',"md",true)  
end
local Message_Reply = LuaTele.getMessage(msg.chat_id, msg.reply_to_message_id)
...
return LuaTele.sendText(msg_chat_id,msg_id,'⋆ تم استرجاع المجموعات من نسخه العزايزي')
else
...
if text == 'المبرمج عزايزي' or text == 'ملو' or text == 'ملاذ' then  
...
if text == 'مبرمج السورس' or text == 'امير' or text == 'بدبد' then  
...
local Namebot = (Redis:get(Alazizy.."Name:Bot") or "العزايزي")
local BotName = {
...
if text == (Redis:get(Alazizy.."Name:Bot") or "العزايزي") then
local photo = LuaTele.getUserProfilePhotos(Alazizy)
local ban = LuaTele.getUser(Alazizy)
local Namebot = (Redis:get(Alazizy.."Name:Bot") or "العزايزي")
local BotName = {
...
local CmdStart = '*\n⋆ أهلآ بك في بوت '..(Redis:get(Alazizy.."Name:Bot") or "العزايزي")..
...
{
{text = 'اذاعه بالتوجيه ⋆ ',type = 'text'},{text = 'ذاعه بالتثبيت ⋆ ', type = 'text'},
},
{
{text = '⋆ القائمه الرئيسية • ',type = 'text'},
},
...
if text == 'المبرمج عزايزي' or text == 'ملاذ' or text == 'المبرمج' or text == 'ملو' then  
local UserId_Info = LuaTele.searchPublicChat("php_7")
if UserId_Info.id then
local  ban = LuaTele.getUser(UserId_Info.id)
local  bain = LuaTele.getUserFullInfo(Sudo_Id)
